...
export async function askLuna(options: {
  request: string;
  duration: number;
  currentTime: number;
  selection: Selection;
  ...
  if (!response.ok) throw new Error(`Luna request failed: ${payload.error?.message ?? `HTTP ${response.status}`}`);
  return parseLunaResponse(payload, model);
}

export function parseLunaResponse(payload: ResponsePayload, model = DEFAULT_MODEL): LunaDecision {
  ...
  if (!call) {
    ...
    throw new Error("Luna did not return an edit or explanation.");
  }
  let args: Record<string, unknown>;
  try { args = JSON.parse(call.arguments) as Record<string, unknown>; }
  catch { throw new Error("Luna returned malformed edit arguments."); }
  ...
  if (call.name === "change_speed") {
    ...
    if (factor < 0.25 || factor > 16) throw new Error("Luna returned a speed outside 0.25x–16x.");
  ...
  if (call.name === "answer_user") {
    if (typeof args.message !== "string" || !args.message.trim()) throw new Error("Luna returned an empty answer.");
    return { kind: "message", message: args.message.trim(), model };
  }
  throw new Error(`Luna selected an unknown action: ${call.name}`);
}

function buildRequest(options: Parameters<typeof askLuna>[0], model: string) {
...
function finiteNumber(value: unknown, label: string): number { const number = Number(value); if (!Number.isFinite(number)) throw new Error(`Luna returned an invalid ${label}.`); return number; }
function integer(value: unknown, label: string): number { const number = finiteNumber(value, label); if (!Number.isInteger(number) || number < 0) throw new Error(`Luna returned an invalid ${label}.`); return number; }
...
function validateRange(value: Record<string, unknown>) { const start = finiteNumber(value.start, "start time"); const end = finiteNumber(value.end, "end time"); if (Math.abs(end - start) < 0.01) throw new Error("Luna returned an empty time range."); return { start: Math.min(start, end), end: Math.max(start, end) }; }
function validateRanges(value: unknown) { if (!Array.isArray(value) || value.length === 0) throw new Error("Luna returned no time ranges."); return value.map((range) => { if (!range || typeof range !== "object") throw new Error("Luna returned an invalid time range."); return validateRange(range as Record<string, unknown>); }); }
